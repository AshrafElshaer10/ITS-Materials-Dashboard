...
# Data Extraction - Stage-01
df_num = pd.read_csv(r"Data_Store.csv", usecols=['QTY._As _Per_Contract','BAC = Budget @ Completion (EGP)',
                                                'Actual_QTY.','Actual_Unit_Cost(EGP)','Targeted_Unit_Sale(EGP)',
                                                'Qty._Variance', 'Total_Expected_Sales(EGP)','Total_Targeted_Sales(EGP)','Sales_Variance(EGP)',
                                                'AC = Actual_Cost_To_Date(EGP)','EV = Earned Value (EGP)','CPI = Cost Performance index',
                                                'CV = Cost Variance (EGP)','TCPI = To Complete Performance index','EAC = Estimated @ Completion (EGP)',
                                                'ETC = Estimate Cost to complete (EGP)','VAC = Variance @ Completion (EGP)','Total_Targeted_Gross_Profit(EGP)',
                                                'Total_Expected_Gross_Profit(EGP)','Actual_Sales_Progress(EGP)','Cash_in(EGP)','Cash_out(EGP)'])
# Data Extraction - Stage02
df_extracted = df_num[['EV = Earned Value (EGP)','Qty._Variance','AC = Actual_Cost_To_Date(EGP)','Total_Expected_Sales(EGP)','Actual_Sales_Progress(EGP)',
                    'Total_Targeted_Gross_Profit(EGP)','Total_Expected_Gross_Profit(EGP)']]
# Removing Oultiers
Q1 = df_extracted['EV = Earned Value (EGP)'].quantile(0.25)
Q3 = df_extracted['EV = Earned Value (EGP)'].quantile(0.75)
IQR = Q3 - Q1
...
axes[0,0].set_ylabel('Total Expected Gross Profit (EGP)')

sns.regplot(x = df_extracted_cleaned['Qty._Variance'], y = df_extracted_cleaned['Total_Expected_Gross_Profit(EGP)'], ax = axes[0,1], line_kws={"color": "red"})
axes[0,1].set_title('Qty._Variance vs Total Expected Gross Profit')
axes[0,1].set_xlabel('Qty._Variance')
axes[0,1].set_ylabel('Total Expected Gross Profit (EGP)')

sns.regplot(x = df_extracted_cleaned['AC = Actual_Cost_To_Date(EGP)'], y = df_extracted_cleaned['Total_Expected_Gross_Profit(EGP)'], ax = axes[0,2], line_kws={"color": "red"})
axes[0,2].set_title('Actual Cost vs Total Expected Gross Profit')
axes[0,2].set_xlabel('Actual Cost(EGP)')
axes[0,2].set_ylabel('Total Expected Gross Profit (EGP)')

sns.regplot(x = df_extracted_cleaned['EV = Earned Value (EGP)'], y = df_extracted_cleaned['Total_Targeted_Gross_Profit(EGP)'], ax = axes[1,0], line_kws={"color": "red"})
axes[1,0].set_title('Earned Value vs Total_Targeted_Gross_Profit')
axes[1,0].set_xlabel('Earned Value (EGP)')
axes[1,0].set_ylabel('Total_Targeted_Gross_Profit(EGP)')

sns.regplot(x = df_extracted_cleaned['Qty._Variance'], y = df_extracted_cleaned['Total_Targeted_Gross_Profit(EGP)'], ax = axes[1,1], line_kws={"color": "red"})
axes[1,1].set_title('Qty._Variance vs Total_Targeted_Gross_Profit')
axes[1,1].set_xlabel('Qty._Variance')
axes[1,1].set_ylabel('Total_Targeted_Gross_Profit(EGP)')

sns.regplot(x = df_extracted_cleaned['AC = Actual_Cost_To_Date(EGP)'], y = df_extracted_cleaned['Total_Targeted_Gross_Profit(EGP)'], ax = axes[1,2], line_kws={"color": "red"})
axes[1,2].set_title('Actual Cost vs Total_Targeted_Gross_Profit')
