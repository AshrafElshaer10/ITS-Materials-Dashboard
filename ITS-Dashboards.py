...
with st.form(key ="input_form", clear_on_submit= True): # "clear_on_submit" to remove inserted values & be ready for new vlaues
    ...
    database_without_calculations['Total_Targeted_Sales(EGP)'] = (database_without_calculations['Actual_QTY.'] * database_without_calculations['Targeted_Unit_Sale(EGP)']).round(2)
    ...
    database_without_calculations['Total_Expected_Gross_Profit(EGP)'] = (database_without_calculations['Total_Targeted_Gross_Profit(EGP)'] + database_without_calculations['CV = Cost Variance (EGP)']).round(2)
    database_without_calculations['Actual Sales(EGP)'] = (database_without_calculations['Total_Targeted_Sales(EGP)'] * database_without_calculations['Progress (%)']).round(2)
    database_without_calculations['Actual_To_Date_Profit (EGP)'] = (database_without_calculations['Actual Sales(EGP)'] - database_without_calculations['AC = Actual_Cost_To_Date(EGP)']).round(2)
    ...
    if len(database_without_calculations) > 0:    # Return No. of rows
        database_without_calculations['Cash_in(EGP)'].iloc[0] = database_without_calculations['Actual Sales(EGP)'].iloc[0] # As 1st item hasn't previous value to accumulate
    ...
    for i in range(1, len(database_without_calculations['Description'])):
        database_without_calculations['Cash_in(EGP)'].iloc[i] = database_without_calculations['Cash_in(EGP)'].iloc[i-1] + database_without_calculations['Actual Sales(EGP)'].iloc[i]
...
a1.metric(':moneybag: :white[BAC] :gray-background[Budget @ Comp. (EGP)]', numerize.numerize(a1_metric_value_rounded), help ="The value of total planned work, the project cost baseline")
a2_metric_value_sum = a2_metric_value.sum()
a2_metric_value_rounded = round(a2_metric_value_sum, 2)
a2.metric('💸:white[AC] :gray-background[Actual Cost To Date (EGP)]', numerize.numerize(a2_metric_value_rounded), help ="The actual cost of all the work completed to a point in time, usually the data date")
a3_metric_value_sum = a3_metric_value.sum()
a3_metric_value_rounded = round(a3_metric_value_sum, 2)
a3.metric(':credit_card: :white[EAC] :gray-background[Estimated cost @ Completion (EGP)]', numerize.numerize(a3_metric_value_rounded), help = "If future work will be accomplished at the planned rate \t :violet[(EAC = AC + BAC - EV)]")
a4_metric_value_sum = a4_metric_value.sum()
a4_metric_value_rounded = round(a4_metric_value_sum, 2)
a4.metric(':chart: :white[EV] :gray-background[Earned Value (EGP)]', numerize.numerize(a4_metric_value_rounded), help ="The planned value of all the work completed (earned) to a point in time, usually the data date, without reference to actual costs")
...
a5.metric(':bar_chart: :white[CV] :gray-background[Cost Variance (EGP)]', numerize.numerize(a5_metric_value_rounded), delta = cv_delta, delta_color='normal', help ="Positive = Under planned cost, Neutral = On planned cost, Negative = Over planned cost \t :violet[(CV = EV-AC)]")
...
a6.metric(':briefcase: :white[VAC] :gray-background[Variance @ Completion (EGP)]', numerize.numerize(a6_metric_value_rounded), delta = vac_delta, delta_color='normal', help = "Positive = Under planned cost, Neutral = On planned cost, Negative = Over planned cost \t :violet[(VAC = BAC-EAC)]")

# 2nd Row
b1_metric_value_sum = b1_metric_value.sum()
b1_metric_value_rounded = round(b1_metric_value_sum, 2)
b1.metric(':dollar: :white[ETC] :gray-background[Estimate Cost to complete (EGP)]', numerize.numerize(b1_metric_value_rounded), help = "The expected cost to finish all the remaining project work \t :violet[(ETC = EAC-AC)]")
b2_metric_value_mean = b2_metric_value.mean()
b2_metric_value_rounded = round(b2_metric_value_mean, 2)
b2.metric(':card_index: :wh2ite[CPI] :gray-background[Cost Performance index]', b2_metric_value_rounded, delta = CPI_delta, delta_color='normal', help ="Greater than 1.0 = Under planned cost, Exactly 1.0 = On planned cost, Less than 1.0 = Over planned cost \t :violet[(CPI = EV/AC)]")
b3_metric_value_mean = b3_metric_value.mean()
b3_metric_value_rounded = round(b3_metric_value_mean, 2)
b3.metric(':card_index_dividers: :white[TCPI] :gray-background[To Complete Performance index]', b3_metric_value_rounded, delta = TCPI_delta, delta_color='normal', help ="Greater than 1.0 = Harder to complete, Exactly 1.0 = Same to complete, Less than 1.0 = Easier to complete \t :violet[TCPI = (BAC-EV)/(BAC-AC)]")
b4_metric_value_sum = b4_metric_value.sum()
b4_metric_value_rounded = round(b4_metric_value_sum, 2)
b4.metric(':dart: :white[Target Profit(EGP)] :gray-background[Total_Targeted_Gross_Profit(EGP)]', numerize.numerize(b4_metric_value_rounded), help = "The value of total planned caluses profit, the project profit baseline")
b5_metric_value_sum = b5_metric_value.sum()
b5_metric_value_rounded = round(b5_metric_value_sum, 2)
b5.metric(':heavy_dollar_sign: :white[To_Date_Profit(EGP)] :gray-background[Actual_To_Date_Profit (EGP)]', numerize.numerize(b5_metric_value_rounded), help = "Actual Obtained Profit")
...
with c1:
    ...
    num_filter1 = num_filter1.selectbox('Values:', ['Total_Expected_Gross_Profit(EGP)','Progress (%)','BAC = Budget @ Completion (EGP)','Qty._Variance','Total_Targeted_Sales(EGP)',
                                                    'Sales_Variance(EGP)','AC = Actual_Cost_To_Date(EGP)','EV = Earned Value (EGP)','CPI = Cost Performance index',
                                                    'TCPI = To Complete Performance index','CV = Cost Variance (EGP)','EAC = Estimated @ Completion (EGP)',
                                                    'ETC = Estimate Cost to complete (EGP)','VAC = Variance @ Completion (EGP)','Total_Targeted_Gross_Profit(EGP)',
                                                    'Actual Sales(EGP)'], key='num_filter1') ## Adding key parameter to each st.select box call to ensure each widget has a unique key for c1, c2, c3.
...
with c2:
    ...
    num_filter2 = num_filter2.selectbox('Values:', ['Total_Expected_Gross_Profit(EGP)','Progress (%)','BAC = Budget @ Completion (EGP)','Qty._Variance','Total_Targeted_Sales(EGP)',
                                                    'Sales_Variance(EGP)','AC = Actual_Cost_To_Date(EGP)','EV = Earned Value (EGP)','CPI = Cost Performance index',
                                                    'TCPI = To Complete Performance index','CV = Cost Variance (EGP)','EAC = Estimated @ Completion (EGP)',
                                                    'ETC = Estimate Cost to complete (EGP)','VAC = Variance @ Completion (EGP)','Total_Targeted_Gross_Profit(EGP)',
                                                    'Actual Sales(EGP)'], key='num_filter2') ## Adding key parameter to each st.select box call to ensure each widget has a unique key for c1, c2, c3.
...
with c3:
    ...
    num_filter3 = num_filter3.selectbox('Values:', ['EV = Earned Value (EGP)','Total_Expected_Gross_Profit(EGP)','Progress (%)','BAC = Budget @ Completion (EGP)','Qty._Variance','Total_Targeted_Sales(EGP)',
                                                    'Sales_Variance(EGP)','AC = Actual_Cost_To_Date(EGP)','CPI = Cost Performance index',
                                                    'TCPI = To Complete Performance index','CV = Cost Variance (EGP)','EAC = Estimated @ Completion (EGP)',
                                                    'ETC = Estimate Cost to complete (EGP)','VAC = Variance @ Completion (EGP)','Total_Targeted_Gross_Profit(EGP)',
                                                    'Actual Sales(EGP)'], key='num_filter3') ## Adding key parameter to each st.select box call to ensure each widget has a unique key for c1, c2, c3.
...
y_axis_num_filter = f2.selectbox('Y_axis_Value:', ['Total_Expected_Gross_Profit(EGP)','Progress (%)','BAC = Budget @ Completion (EGP)','Qty._Variance','Total_Targeted_Sales(EGP)',
                                                    'Sales_Variance(EGP)','AC = Actual_Cost_To_Date(EGP)','EV = Earned Value (EGP)','CPI = Cost Performance index',
                                                    'TCPI = To Complete Performance index','CV = Cost Variance (EGP)','EAC = Estimated @ Completion (EGP)',
                                                    'ETC = Estimate Cost to complete (EGP)','VAC = Variance @ Completion (EGP)','Total_Targeted_Gross_Profit(EGP)',
                                                    'Actual Sales(EGP)'])
num_filter = f3.selectbox('Numerical Filtering:', ['Total_Expected_Gross_Profit(EGP)','Progress (%)','BAC = Budget @ Completion (EGP)','Qty._Variance','Total_Targeted_Sales(EGP)',
                                                   'Sales_Variance(EGP)','AC = Actual_Cost_To_Date(EGP)','EV = Earned Value (EGP)','CPI = Cost Performance index',
                                                   'TCPI = To Complete Performance index','CV = Cost Variance (EGP)','EAC = Estimated @ Completion (EGP)',
                                                   'ETC = Estimate Cost to complete (EGP)','VAC = Variance @ Completion (EGP)','Total_Targeted_Gross_Profit(EGP)',
                                                   'Actual Sales(EGP)'])
